...
static metadata_t *split_block(metadata_t *block, size_t size){
  metadata_t *var = (metadata_t *) ((uint8_t *) block + (block -> size) - size);
  var -> size = size;
 ...
 * Simply search through the freelist, looking for a block whose address matches the provided block's address.
...
static metadata_t *find_best_fit(size_t size) {
  ...
  metadata_t *curr = address_list;
  metadata_t *finalBlock;
  long unsigned int finalSize = 0;
  ...
  while (curr != NULL) {
    ...
    if (curr -> size > size && finalSize == 0) {
      finalBlock = curr;
      finalSize = curr -> size;
    }
    if (curr -> size > size && (curr -> size ) < finalSize) {
      finalBlock = curr;
      finalSize = curr -> size;
    }
    curr = curr -> next;
...
void *my_malloc(size_t size) {
    ...
    if (block -> size >= TOTAL_METADATA_SIZE + size + 1) {
      metadata_t *newBlock = split_block(block, size);
      return newBlock + 1;
    }
