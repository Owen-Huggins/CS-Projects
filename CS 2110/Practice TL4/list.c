...
{
    struct user *test;
    test = (struct user *) malloc(sizeof(struct user));
    if (test == NULL) {
      return NULL;
    }
    ...
    if (name == NULL) {
      ...
      if (test -> name == NULL) {free(test); return NULL;}
    ...
    if (type == STUDENT) {
      ...
      if (student1 == NULL) {
        free(test -> name);
        free(test -> next);
        free(test -> data.student.grades);
        free(test);
        free(student1);
        return NULL;
      }

      int studentTest = create_student(data.student.num_classes, data.student.grades, student1);
      if (studentTest == 1) {
        free(test -> name);
        free(test -> next);
        free(test -> data.student.grades);
      ...
      instructor1 = (struct instructor *)malloc(sizeof(struct instructor));
      if (instructor1 == NULL) {
        free(test -> name);
        free(test -> next);
        free(test);
      ...
      int teacherTest = create_instructor(data.instructor.salary, instructor1);
      if (teacherTest == 1) {
        free(test -> name);
        free(test -> next);
        free(test);
        ...
        test -> data.instructor = *instructor1;
        free(instructor1);
...
static int user_equal(const struct user *user1, const struct user *user2)
{
    ...
    enum user_type choice1 = user1 -> type;
    enum user_type choice2 = user2 -> type;
    if (choice1 != choice2) {
      return 0;
    } else if (choice1 == STUDENT) {
...
struct user_list *create_list(void)
{
    ...
    list = malloc(sizeof(struct user_list));
    if (list == NULL) {
      return NULL;
    }
    list -> size = 0;
...
int get(struct user_list *list, int index, struct user **dataOut)
{
    if (dataOut == NULL || list == NULL) {
      return 1;
    } else if (list -> size <= index || index < 0) {
      *dataOut = NULL;
      return 1;
    }
    struct user *curr = list -> head;
...
void empty_list(struct user_list *list)
{
    ...
    int counter = 0;
    while (counter < list -> size && curr != NULL) {
      next = curr -> next;
...
int get_highest_paid(struct user_list *list, struct user **dataOut)
{
  ...
  int val = 0;
  struct user *highInstructor = NULL;
  struct user *curr = list -> head;
  int counter = -1;
  while (counter < list -> size) {
    if (curr -> type == INSTRUCTOR) {
      if (curr -> data.instructor.salary > val + 1) {
        val = curr -> data.instructor.salary;
        highInstructor = curr;
      }
    }
      counter++;
